-- Add a popularity column to boost frequently-selected universities in
-- search results. Backfill from current profile data.
ALTER TABLE universities ADD COLUMN IF NOT EXISTS popularity integer NOT NULL DEFAULT 0;

-- Backfill: count how many profiles reference each university name.
UPDATE universities u
SET popularity = sub.cnt
FROM (
  SELECT p.university AS name, count(*) AS cnt
  FROM profiles p
  WHERE p.university IS NOT NULL AND p.university != ''
  GROUP BY p.university
) sub
WHERE u.name = sub.name;

-- Drop + recreate to update the function body (existing version has
-- parameter defaults that CREATE OR REPLACE cannot remove).
DROP FUNCTION IF EXISTS search_universities(text, int, text);

CREATE FUNCTION search_universities(_q text, _limit int, _city text)
...
LANGUAGE sql STABLE AS $$
  ...
  base AS (
    SELECT u.id, u.name, u.city, u.country,
      CASE
        WHEN q.term = '' THEN 10 + u.popularity * 2
        ELSE GREATEST(
