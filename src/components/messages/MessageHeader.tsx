      ...
      {profile && (
        <div className="flex items-center gap-3 flex-1">
          {profile.id && (
            <Link to={`/profile/${profile.id}`} className="hover:opacity-80 transition-opacity">
              <Avatar className="h-12 w-12 cursor-pointer hover:ring-2 hover:ring-erasmatch-blue transition-all">
                ...
                <AvatarFallback className="bg-gradient-to-br from-erasmatch-blue to-erasmatch-purple text-white">
          ...
          <div>
            ...
            {profile.university && <div className="text-xs text-gray-500">{profile.university}</div>}
          </div>
        </div>
      )}
    </div>
