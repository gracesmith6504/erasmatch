...
export const HeroSection = ({ handleFindStudents }: HeroSectionProps) => {
  ...
  useEffect(() => {
    supabase.rpc("get_featured_activity_profiles").then(({ data }) => {
      if (data) {
        setFeaturedAvatars(data.filter((p) => p.avatar_url).map((p) => p.avatar_url!));
      }
    });
  }, []);
          ...
          >
            ...
            >
              <div className="text-center sm:text-left">
                <p className="text-xl sm:text-2xl font-bold text-foreground">700+</p>
